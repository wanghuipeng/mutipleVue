  ...
  <div class="content">
    <div class="header">
      <div class="header-info" @click="toDemo">
        ...
        <p>
          ...
          <span v-if="weather">{{ weather.today.weatherType }}，{{ weather.today.low }}-{{ weather.today.high }}度</span>
        </p>
      </div>
      <div class="header-search">
        <p class="tab">
          <span :class="{active: actIdx === 1}" @click="switchTab(1)">职位</span>
          <span :class="{active: actIdx === 2}" @click="switchTab(2)">候选人</span>
        </p>
        <p class="search">
          ...
          <el-autocomplete
            v-model="search"
            value-key="text"
            :placeholder="`${actIdx === 1 ? '搜索职位名称' : '人才姓名、手机号搜索'}`"
            size="medium"
            clearable
      ...
      <div class="header-btns">
        <el-button v-if="userInfo.companyRole === 2 || userInfo.companyRole === 3 || userInfo.companyRole === 4" size="mini" plain @click="releasePosition">新增职位</el-button>
    ...
    <el-card class="task" shadow="never">
      <div class="title">
        <p>待处理</p>
        <p>
          <el-select v-model="taskPositionId" size="mini" placeholder="请选择" filterable @change="getTaskCount">
            <el-option
          ...
          <el-select v-model="taskDateType" size="mini" placeholder="请选择" @change="getTaskCount">
            <el-option
      ...
      <ul class="task-list">
        <li v-for="(item, index) in taskList" :key="index" @click="toCandidateList(item)">
          <p>{{ item.name }}</p>
          <p class="num">
    ...
    <div class="main">
      <el-card class="main-left" shadow="never">
        <div class="title">
          <p>重要事项</p>
          <el-select v-model="orderStage" size="mini" placeholder="请选择" @change="getList">
            <el-option
        ...
        <div class="calendar">
          ...
          <div class="box" style="overflow: hidden; flex: 1;">
            <ul v-if="weekList && weekList.length > 0" class="week" :style="[{'margin-right': distance}, {'transition': 'all .3s ease-out .1s'}]">
              <li v-for="(item, index) in weekList" :key="index" class="week-item">
                <p class="name">{{ item.week }}</p>
                <div class="value" @click="changeDay(item)">
                  <p :class="{active: checkDay === item.date}">{{ item.month ? item.month + '/' : '' }}{{ item.day }}</p>
        ...
        <el-timeline v-if="scheduleList.length > 0" class="timeline">
          <el-timeline-item
            ...
            :key="index"
          >
            <div class="interview" @click="toDetail(item.resumeId, item.candidateId)">
              ...
              <div class="interviewee">
                ...
                <p v-if="item.processNode === 2">{{ item.interviewMethod | filterArr(dictData.dicts.interviewType) }}：<span style="color: #1678F9;">{{ item.positionName }}(面试官：{{ item.interviewerName }})</span></p>
                <p v-if="item.processNode === 4">入职岗位：<span style="color: #1678F9;">{{ item.positionName }}(试用期：{{ item.probationPeriod }}个月)</span></p>
              </div>
            </div>
          </el-timeline-item>
        </el-timeline>
      ...
      <div class="main-right">
        <el-card class="position" shadow="never">
          <div class="title">
            <p>最近访问职位</p>
            <span @click="morePosition">更多在招职位<i class="el-icon-arrow-right"/></span>
          </div>
          <ul v-if="browsePositionList.length > 0">
            <li v-for="item in browsePositionList" :key="item.id" class="ellipsis" @click="toPosition(item.id)">{{ item.title }}({{ item.id }})</li>
          </ul>
        ...
        <el-card class="echart" shadow="never">
          <div class="title">
            <p>招聘趋势</p>
            <el-select v-model="dateType" size="mini" placeholder="请选择" @change="getCharts">
              <el-option
...
export default {
  ...
  data() {
    return {
      ...
      taskDateType: null,
      dateDicts: [
        { label: '全部', value: null }, { label: '今天', value: 1 }, { label: '近3天', value: 3 },
        { label: '近7天', value: 7 }, { label: '近30天', value: 30 }
      ],
      orderStage: '',
      orderType: [
        { label: '全部', value: '' }, { label: '面试', value: 2 },
        { label: '入职', value: 4 }
      ],
      dateType: 7,
      dateDicts1: [
        { label: '近3天', value: 3 }, { label: '近7天', value: 7 },
        { label: '近30天', value: 30 }
      ],
      browsePositionList: [],
  ...
  methods: {
    ...
    getInfo() {
      ...
      getWorkbenchInfo(params).then(res => {
        ...
        this.nowDate = day.replace(/-/, '年').replace(/-/, '月') + '日 ' + week.week
        this.ipLbs = res.data.ipLbs
        this.weather = res.data.weather
        this.browsePositionList = res.data && res.data.browsePositionList.splice(0, 6)
      })
    },
    // 切换Tab
    switchTab(val) {
