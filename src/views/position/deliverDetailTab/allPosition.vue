<template>
  <div class="wrap">
    ...
    <div class="class-search address-search">
      ...
      <span class="result">
        <el-link
          v-for="item in teacherPositionAddress"
          :key="item.value"
          :underline="false"
    ...
    <div class="class-search profession-search">
      ...
      <span class="result">
        <el-link
          v-for="item in teacherPositionFunctionType"
          :key="item.value"
          :underline="false"
...
import {
  getPositionList,
  getTeacherPositionAddress,
  getTeacherPositionFunctionType
} from '@/api/position'
export default {
  components: {
    positionItem,
  ...
  data() {
    return {
      // 分析师职位地点
      teacherPositionAddress: [],
      // 分析师职位职能
      teacherPositionFunctionType: [],
      // 查询条件
      queryForm: {
  ...
  mounted() {
    // 查询分析师职位地点
    this.queryTeacherPositionAddress()
    // 查询分析师职位职能
    this.queryTeacherPositionFunctionType()
  },
  methods: {
    // 查询分析师职位地点
    queryTeacherPositionAddress() {
      const para = {}
      para.cwUserId = this.userInfo.id
      getTeacherPositionAddress(para).then(res => {
        this.teacherPositionAddress = res.data
      })
    },
    // 查询分析师职位职能
    queryTeacherPositionFunctionType() {
      const para = {}
      para.cwUserId = this.userInfo.id
      getTeacherPositionFunctionType(para).then(res => {
        this.teacherPositionFunctionType = res.data
      })
    },
    // 选择地点
