<template >
  <div
    ...
    ref="leftCont"
    v-loading="loading"
    :class="[ $route.name === 'resumeDetail' ? 'resume-detail-new mtv-25' : 'resume-detail-new']"
  >
    <!-- 头部 -->
    <div
    ...
    <!-- 推荐报告 -->
    <div v-if="reportDetail && (userInfo && userInfo.companyAccountType === 2)" class="report">
      <h3>推荐报告</h3>
      <el-form v-if="reportDetail" label-width="95px" :inline="true">
        <el-form-item label="推荐时间">{{ reportDetail.createTime }}</el-form-item>
        <el-form-item v-if="reportDetail.currentSalary" label="目前年薪">
          <span v-if="reportDetail.currentSalaryTax === 1">税前</span>
          <span v-else-if="reportDetail.currentSalaryTax === 2">税后</span>
          {{ parseInt(reportDetail.currentSalary) }}元*{{ reportDetail.currentSalaryMonth }}个月
        </el-form-item>
        <el-form-item
          v-if="reportDetail.currentOtherSalary"
          label="其他收入"
        >{{ reportDetail.currentOtherSalary }}</el-form-item>
        <el-form-item
          v-if="reportDetail.motivation"
          label="求职动机"
        >{{ reportDetail.motivation | filterArr( dictData.dicts.jobMotivation) }}</el-form-item>
        <el-form-item v-else label="求职动机">-</el-form-item>
        <el-form-item
          v-if="reportDetail.dimissionDate"
          label="预计离职期"
        >{{ reportDetail.dimissionDate | filterArr( dictData.dicts.dimissionDate) }}</el-form-item>
        <el-form-item v-else label="预计离职期">-</el-form-item>
        <el-form-item
          v-if="reportDetail.expectInterviewTime"
          label="面试时间"
        >{{ reportDetail.expectInterviewTime || '-' }}</el-form-item>
        <el-form-item label="推荐理由" class="pre-line">{{ reportDetail.reason|| '-' }}</el-form-item>
      </el-form>
    </div>
    <!-- 基本信息 -->
    <div
    ...
    >
      <h3 :class="[ !baseInfoVisible && editDisabled ? 'disabled-h3':'']">
        ...
        <span
          ...
          :class="[editDisabled ? 'disabled edit' : 'edit']"
          @click="editIt('baseInfo')"
        >编辑</span>
      </h3>
      <!-- 文本展示 -->
    ...
    <div class="resume-mess">
      ...
      <div v-if="detailData.detail" class="jobIntent">
        <h3 id="apply-intent" :class="[!jobIntentVisible && editDisabled ? 'disabled-h3':'']">
          ...
          >不完善</label>-->
          <span
            v-if="!jobIntentVisible"
            :class="[editDisabled ? 'disabled edit' : 'edit']"
            @click="editIt('jobIntent')"
          >编辑</span>
        </h3>
        <!-- 文本展示 -->
        ...
        <div v-show="jobIntentVisible" class="edit-form">
          ...
          >
            ...
            <el-form-item
              label="期望年薪"
              prop="intentionSalary"
              class="salary-group intent-salary-group"
            >
              <el-input v-model="jobIntentForm.intentionSalary" placeholder="年薪" class="mr-0">
      ...
      <div class="eduExp">
            ...
            >
              ...
              <div class="cont">
                <p class="part-1">
                  ...
                  <span v-if="item.startDate" class="time">{{ item.startDate }}-{{ item.endDate }}</span>
                </p>
                <p class="part-item">
                  <label>学历</label>
                  {{ item.degreeCode | filterArr(degreeArrTemp) }}
                </p>
                <p class="part-item">
                  <label>专业</label>
                  {{ item.speciality }}
        ...
        <div v-if="false" class="add-btn add-btn-disabled">
          <i class="el-icon-plus"></i> 添加教育经历
        </div>
      </div>
      <!-- 工作经历 -->
          ...
          >
            ...
            >
              <div class="part-1">
                ...
                <p class="time">
                  {{ item.startDate }}
                  <label v-if="item.startDate">-</label>
                  {{ item.endDate || (item.isNow ? '至今' : '') }}
                  <label
                    v-if="item.startToEndDate"
                  >（{{ item.startToEndDate }}）</label>
                </p>
                <label class="edit" @click="editIt('workExp',i)">编辑</label>
              </div>
              <el-form label-width="95px" :inline="true">
                <el-form-item
                ...
                <div class="take-date">
                  ...
                  <el-form-item prop="endDate" class="end-date">
                    <el-date-picker
                      ...
                      value-format="yyyy-MM"
                      type="month"
                      popper-class="to-now-date"
                      @change="changeToNowDate"
                      @focus="focusToNowDate"
                      @blur="blurToNowDate"
                    ></el-date-picker>
                    <span v-if="workExpForm.isNow" class="to-now-text">至今</span>
                  </el-form-item>
                </div>
        ...
        <div class="text-form">
          ...
          >
            ...
            >
              ...
              <div class="cont-1">
                <p class="part-1">
                  <span class="school" :title="item.trainingCourse">{{ item.trainingCourse }}</span>
        ...
        <div class="text-form">
          <div v-for="(item,i) in detailData.languages" :key="i" class="item-inner">
            ...
            >
              ...
              <div class="cont-1">
                <p class="part-1">
                  <span class="school">{{ item.languages }}</span>
                </p>
                <p v-if="item.level" class="part-item">
                  <label>等级</label>
                  {{ item.level }}
                </p>
                <p class="part-item">
                  <label>口语能力</label>
                  {{ item.listeningSpeakingSkills | filterArr(readListenSkillsArr) }}
                </p>
                <p class="part-item">
                  <label>书面能力</label>
                  {{ item.readingWritingSkills | filterArr(readListenSkillsArr) }}
                </p>
              </div>
            </div>
            <!-- 表单编辑 -->
        ...
        <div class="text-form">
          <div v-for="(item,i) in detailData.cerInfos" :key="i" class="item-inner">
            ...
            >
              ...
              <div class="cont-1">
                <p class="part-1">
                  <span class="school">{{ item.nameOfCertificate }}</span>
                </p>
                <p class="part-item">
                  <label>获证时间</label>
                  {{ item.receivingDate }}
                </p>
                <p v-if="item.score" class="part-item">
                  <label>描述</label>
                  {{ item.score }}
                </p>
              </div>
      ...
      <div v-if="detailData.detail" class="selfEval">
        ...
        <div class="text-form">
          ...
          <!-- 表单编辑 -->
          <div v-else class="edit-form">
            <el-form ref="selfEvalForm" :model="selfEvalForm" label-width="95px" :inline="true">
  ...
  data() {
    return {
      ...
      workExpFormRules: {
        ...
        endDate: [
          { required: this.requiredOr, message: '请选择结束时间', trigger: 'change' }
        ],
        companyName: [
  ...
  computed: {
    ...
    reportDetail() {
      return (
        ...
        this.$store.state.customer.reportDetail &&
        this.$store.state.customer.reportDetail.candidateRecommendReport
      )
    }
  },
  destroyed() {
  ...
  methods: {
    ...
    blurToNowDate(e) {
      ...
      if (!e) {
        ...
        this.$set(this.workExpForm, 'isNow', true)
      } else {
        console.log(2)
        if (e && e.$data && !e.$data.valueOnOpen) {
          this.$set(this.workExpForm, 'isNow', true)
        }
      }
      const chArr = document.body.getElementsByClassName('now-date')
    ...
    focusToNowDate() {
      this.$nextTick(() => {
        var ele = document.getElementsByClassName('to-now-date')[0]
        var para = document.createElement('p')
        var node = document.createTextNode('至今')
        para && para.setAttribute('class', 'now-date')
        para && para.appendChild(node)
        var element = ele && ele.getElementsByClassName('el-picker-panel__content')[0]
        element && element.appendChild(para)
        var dom = ele && ele.getElementsByClassName('now-date')
        if (dom) {
          for (let i = 0; i < dom.length; i++) {
              dom[i].onclick = () => {
              this.$set(this.workExpForm, 'isNow', true)
              this.requiredOr = false
              this.$set(this.workExpForm, 'endDate', '')
    ...
    changeToNowDate(e) {
      console.log(111, e)
      this.$set(this.workExpForm, 'endDate', e)
      this.$set(this.workExpForm, 'isNow', false)
    },
    handleCommand(e) {
      if (e.type === 'a') {
    ...
    editIt(name, index) {
      ...
      console.log('每一项的详情', this[name + 'Form'])
    },
    // 添加
    addIt(name) {
      this.editDisabled = true
    ...
    editResumeExperience(values, id) {
      ...
      params.isNow = this.workExpForm.isNow
...
<style  lang="scss">
@import '@/assets/sass/resume/resumeDetailNew.scss';
</style>
