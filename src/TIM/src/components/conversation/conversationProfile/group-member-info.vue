<template>
  <div>
    <div>
      ...
      <el-popover v-model="popoverVisible" v-show="showBan" title="禁言">
        <el-input
          v-model="muteTime"
          placeholder="请输入禁言时间"
    ...
    <div>
      ...
      {{ member.nameCard || '暂无' }}
      <el-popover v-model="nameCardPopoverVisible" v-show="showEditNameCard" title="修改群名片">
        <el-input
          v-model="nameCard"
          placeholder="请输入群名片"
    ...
    <el-button v-if="canChangeRole" type="text" @click="changeMemberRole">
      {{
        member.role === 'Admin' ? '取消管理员' : '设为管理员'
      }}
    </el-button>
    <el-button v-if="showKickout" type="text" style="color:red;" @click="kickoutGroupMember">踢出群组</el-button>
  </div>
</template>
