<template>
  <div v-if="dialling || calling || isDialled" class="call-container">
    <div v-if="isDialled" class="choose">
      <div class="title">
        {{ toAccountInfo.nickName }}&nbsp;邀请您视频通话
      </div>
      <div class="buttons">
    ...
    <div v-if="dialling || calling" class="call">
      <div v-if="dialling" class="title">
        正在呼叫&nbsp;{{ toAccountInfo.nickName }}...
      </div>
      <div v-show="calling" id="local" :class="isLocalMain ? 'small' : 'big'" @click="changeMainVideo"></div>
      <div v-show="calling" name="remote" :class="isLocalMain ? 'big' : 'small'" @click="changeMainVideo"></div>
      <div v-show="calling" class="duration">
        {{ formatDurationStr }}
      </div>
      <div class="btns">
        <div class="btns-left">
        ...
        <div class="btns-right">
          <p v-if="toAccountInfo.resumeId" class="resume" @click="lookResume">简历</p>
          <p class="end" @click="leave">结束</p>
        </div>
      </div>
      <div v-show="maskShow" class="mask" :class="isLocalMain ? 'small' : 'big'" @click="changeMainVideo">
        <div>
...
import { finishInterview, interviewVideoAnswer } from '@/api/interviewRoom'
export default {
  name: 'CallLayer',
  data() {
    return {
  ...
  computed: {
    ...
    ...mapState({
      ...
      userSig: state => state.userInfo.imUserSig,
      videoRoom: state => state.video.videoRoom,
      toAccountInfo: state => state.conversation.toAccountInfo
    }),
    formatDurationStr() {
  ...
  created() {
    if (document.location.hostname === 'h.dalieyingcai.com') {
      this.sdkAppID = '1400289461'
    } else {
      this.sdkAppID = '1400299637'
    }
  ...
  mounted() {
    console.log(this.toAccountInfo)
    ...
    this.$bus.$on('leaveInterviewRoom', this.videoCallLogOut)
  },
  beforeDestroy() {
    ...
    this.$bus.$off('leaveInterviewRoom', this.videoCallLogOut)
  },
  methods: {
    // 查看简历
    lookResume() {
      this.$bus.$emit('openResume', this.toAccountInfo.resumeId)
    },
    // 面试视频接听
    interviewAnswer() {
      console.log('打出去')
      const params = {
        interviewId: this.toAccountInfo.interviewId
      }
      interviewVideoAnswer(params).then(res => {
        console.log(res)
      })
    },
    // 面试完成
    finish() {
      console.log('结束', this.toAccountInfo)
      this.$emit('endVedio')
      const params = {
        interviewId: this.toAccountInfo.interviewId
      }
      finishInterview(params).then(res => {
        console.log(res)
        // 更新面试剩余时间
        this.$bus.$emit('updateInterviewTime')
      })
    },
    videoCallLogOut() { // 针对，刷新页面，关闭Tab，登出情况下，通话断开的逻辑
      if (this.dialling || this.calling) {
        if (this.toAccountInfo.interviewId) {
          this.leave()
        }
      }
      if (this.isDialled) {
